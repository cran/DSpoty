...
get_several_tracks_features_vectorized<-function(tracks_df, access_token = DSpoty::get_spotify_access_token()){
  ...
  na_index<-sapply(res, is_list) %>% unlist() %>% as_vector() %>% which(.==TRUE)
  ...
  indices<-lapply(c(13, seq(1,11,1), 18), function(x) seq(x,length(unlist(res, recursive = FALSE, use.names = FALSE)), 18))
